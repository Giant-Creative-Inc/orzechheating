...
# GridPane hybrid deployment: apply safe WordPress database upgrades as the site user.

set -Eeuo pipefail
umask 022

readonly HOOK_NAME="postdeploy"
...
log "Checking whether WordPress database upgrades are required"
MULTISITE_STATUS="$(timeout "$GP_DEPLOY_WP_CHECK_TIMEOUT" wp eval 'echo is_multisite() ? "yes" : "no";' --path="$WP_PATH")"
readonly MULTISITE_STATUS
case "$MULTISITE_STATUS" in
  yes)
    log "Updating the multisite WordPress database"
    timeout "$GP_DEPLOY_WP_UPDATE_TIMEOUT" wp core update-db --network --path="$WP_PATH"
    ;;
  no)
    log "Updating the single-site WordPress database"
    timeout "$GP_DEPLOY_WP_UPDATE_TIMEOUT" wp core update-db --path="$WP_PATH"
    ;;
  *)
    fail "Unexpected multisite detection result: $MULTISITE_STATUS"
    ;;
esac

log "WordPress database update completed successfully"
